...
public class Singleupdation {
    ...
    //     return query(first, second, (2 * idx + 1), left, mid, tree)
    //             + query(first, second, (2 * idx + 2), mid + 1, right, tree);

    // }

    public static void update(int index,int val,int left, int right, int idx, ArrayList<Integer>tree)
    {
        if(left==right)
        {
            tree.set(idx,val);
        ...
        if(index<=mid)
        {
            update(index,val,left,mid,(2*idx+1),tree);
        }
        else{
            update(index,val,mid+1,right,(2*idx+2),tree);
        }
